...
<<<<<<< HEAD
infofont = ('times', 8, 'bold')
labelfont = ('times', 9)

def getMotorInfo(data):
    info = "OK"
    if (abs(data.present_load) in range(50,80)) | (data.present_temperature > 65):
        label[int(data.id)].config(fg='yellow')
        info = "Loaded"
    elif (abs(data.present_load) > 80) | (data.present_temperature in range(50,65)):
        label[int(data.id)].config(fg='red')
        info = "OVERLOADED"
    elif data.present_voltage > 15:
...
>>>>>>> 3917f49b620ac667fd1a7bfcdb2e7b2991a0e455
    ...
    for id in ids:
        var[id] = StringVar()
<<<<<<< HEAD
        label[id] = Label(master, width= 15, height= 6, textvariable=var[id], font=labelfont, bg='white')
=======
